...
public class ArticuloR implements I_ArticuloR {
	...
	public List<Articulo> getAll() {
		...
		for (String linea : lineas) {
			...
			try {
				id = Integer.parseInt(campos[0].substring(campos[0].indexOf("=") + 1));
			} catch (Exception e) {
			}
			try {
				descripcion = campos[1].substring(campos[1].indexOf("=") + 1);
			} catch (Exception e) {
			}
			try {
				costo = Double.parseDouble(campos[2].substring(campos[2].indexOf("=") + 1));
			} catch (Exception e) {
			}
			try {
				precio = Double.parseDouble(campos[3].substring(campos[3].indexOf("=") + 1));
			} catch (Exception e) {
			}
			try {
				stock = Integer.parseInt(campos[4].substring(campos[4].indexOf("=") + 1));
			} catch (Exception e) {
			}
			try {
				stockMin = Integer.parseInt(campos[5].substring(campos[5].indexOf("=") + 1));
			} catch (Exception e) {
			}
			try {
				stockMax = Integer.parseInt(campos[6].substring(campos[6].indexOf("=") + 1));
			} catch (Exception e) {
			}
