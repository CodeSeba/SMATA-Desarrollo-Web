...
public class ClienteR implements I_ClienteR {
	...
	public List<Cliente> getAll() {
		...
		for (String linea : lineas) {
			...
			try {
				nombre = campos[1].substring(campos[1].indexOf("=") + 1);
			} catch (Exception e) {
			}
			try {
				apellido = campos[2].substring(campos[2].indexOf("=") + 1);
			} catch (Exception e) {
			}
			try {
				tipoDocumento = TipoDocumento.valueOf(campos[3].substring(campos[3].indexOf("=") + 1));
			} catch (Exception e) {
			}
			try {
				numeroDocumento = campos[4].substring(campos[4].indexOf("=") + 1);
			} catch (Exception e) {
			}
			try {
				direccion = campos[5].substring(campos[5].indexOf("=") + 1);
			} catch (Exception e) {
			}
			try {
				comentarios = campos[6].substring(campos[6].indexOf("=") + 1);
			} catch (Exception e) {
			}
