...
public class ClienteR implements I_ClienteR {
    ...
    @Override public List<Cliente> getByApellido(String apellido) {
    ...
    @Override public List<Cliente> getByApellidoNombre(String apellido, String nombre) {
       return getList(getAll()
                .stream()
                .filter(
                        c->c.getApellido().toLowerCase().contains(apellido)
                        && c.getNombre().toLowerCase().contains(nombre)
                )
        );
    }

    @Override public List<Cliente> getByTipoNumero(TipoDocumento tipo, String numero) {
        if (tipo==null) return new ArrayList<Cliente>();
        return getList(getAll()
                .stream()
                .filter(
                        c->c
    ...
    @Override public List<Cliente> getByDireccion(String direccion) {
        return getList(getAll()
                .stream()
                .filter(c->c.getDireccion().equalsIgnoreCase(direccion))
                );
    }

    @Override public List<Cliente> getLikeDireccion(String direccion) {
        return getList(getAll()
                .stream()
                .filter(c->c.getDireccion().toLowerCase().contains(direccion))
        );
    }
    @Override public Cliente getByFactura(Factura factura) {
        List<Cliente>lista=getList(
                getAll()
                        .stream()
                        .filter(c->c.getId()==factura.getIdCliente()));
        return (lista.isEmpty())?null:lista.get(0);
    }
    @Override public Cliente getByFactura(char letra, int numero) {
        /*
        List<Cliente>lista=getList(
                getAll()
                        .stream()
                        .filter(c->c.getId()==numero));
        return (lista.isEmpty())?null:lista.get(0);   
        */
        return null;
    }
