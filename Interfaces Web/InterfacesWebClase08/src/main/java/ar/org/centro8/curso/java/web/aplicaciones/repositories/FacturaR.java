...
public class FacturaR implements I_FacturaR{
    ...
    public Factura getByLetraNumero(char letra, int numero) {
        List<Factura>lista=getAll()
                .stream()
                .filter(p->p.getLetra()==letra && p.getNumero()==numero)
                .collect(Collectors.toList());
        return (lista==null || lista.isEmpty())?null:lista.get(0);
    }
    @Override public List<Factura> getByFecha(Date date) {
        return getAll()
                .stream()
                .filter(p->p.getFecha()==date)
                .collect(Collectors.toList());
    }
    @Override public List<Factura> getByCliente(Cliente cliente) {
        return getByCliente(cliente.getId());
    }
    @Override public List<Factura> getByCliente(int id) {
        return getAll()
                .stream()
    ...
    @Override public void save(Factura e) {
        if(e==null) return;
    ...
    @Override public void remove(Factura e) {
        if(e==null) return;
    ...
    @Override public void update(Factura e) {
        if(e==null) return;
        String query="update facturas set letra=?, numero=?, fecha=?, monto=?, idCliente=? "
                + "where id=?";
        try (PreparedStatement ps=conn.prepareStatement(query)) {
            ps.setString(1, e.getLetra()+"");
            ps.setInt(2, e.getNumero());
